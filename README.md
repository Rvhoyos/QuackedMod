...
A "lightweight" cross-loader Minecraft mod that adds ducks with simple AI, natural spawns, sounds, items, and hatchable eggs. Supports both Fabric and NeoForge. GeckoLib 5.4.5 for duck model and animations.
...
- **NeoForge:** `21.11.x`
- **Shared libs (both loaders):** GeckoLib `5.4.5`
...
### Fabric client
1. Install a Fabric profile for Minecraft 1.21.11 (via the Fabric installer/launcher).
2. In your game directory (`.minecraft/mods`):
   - Add `geckolib-5.4.5 (fabric).jar`
   - Add `fabric-api (1.21.11).jar`
   - Add `quack-fabric-1.1.4.jar`
>Get from official sources. [Geckolib](https://modrinth.com/mod/geckolib/versions?g=1.21.11&l=fabric&l=neoforge)

### NeoForge client
1. Install NeoForge for 1.21.11.
2. In your game directory (`.minecraft/mods`):
   - Add `geckolib-5.4.5 (neoforge).jar`
   - Add `quack-neoforge-1.1.4.jar`
>Get from official sources. [Geckolib](https://modrinth.com/mod/geckolib/versions?g=1.21.11&l=fabric&l=neoforge)

## Manual Installation (Dedicated Server)

### Fabric or NeoForge Server
1. Place the Fabric or Neoforge server launcher jar for **1.21.11** (e.g. `fabric-server-mc.1.21.11-loader.0.17.2-launcher.1.1.0.jar`).
2. Run once to generate files, accept the EULA in `eula.txt`.
3. Put these in the server `mods/`:
   - `geckolib-5.4.5 (fabric).jar`
   - `fabric-api (1.21.11).jar`
   - `quack-fabric-1.1.4.jar` (or neoforge equivalent)
